...
static void mh_apply_feedback_level(uint8_t level)
{
    (void)drv_rumble_set_level(level);
}

static void mh_apply_led_state(void)
{
    const of_dev_t *led = drv_led_get_dev();
    uint32_t out = 0U;
    uint8_t on = ((g_mh.rgb_r | g_mh.rgb_g | g_mh.rgb_b) != 0U) ? 1U : 0U;

    if ((led != 0) && (led->ops != 0) && (led->ops->write != 0)) {
        (void)led->ops->write(led->priv, &on, 1U, &out);
    }
}

static void mh_reset_outputs(void)
{
    g_mh.ffb_solenoid = 0U;
    g_mh.ffb_rumble = 0U;
    g_mh.rgb_r = 0U;
    g_mh.rgb_g = 0U;
    g_mh.rgb_b = 0U;
    mh_apply_feedback_level(0U);
    mh_apply_led_state();
}
...
{
    uint32_t sent = 0;
    (void)of_link_send_serial((const uint8_t *)s, (uint32_t)strlen(s), &sent);
}
    ...
    if ((len >= 2U) && (buf[0] == 'F')) {
        switch (buf[1]) {
            case '0':
                ...
                (void)drv_solenoid_fire_level(g_mh.ffb_solenoid);
                break;
            case '1':
                g_mh.ffb_rumble = (len >= 4U) ? mh_hex_byte(buf, len, 2U) : 0U;
                mh_apply_feedback_level(g_mh.ffb_rumble);
                break;
            case '2':
