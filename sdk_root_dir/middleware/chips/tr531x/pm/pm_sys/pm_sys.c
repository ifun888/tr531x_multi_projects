...
#define TASK_PRIORITY_PM_SYS    30
#define PM_SYS_STACK_SIZE       0xA00
...
char pm_reverse_state[PM_STATE_TRANS_NUM][PM_STATE_LEN] =
{
    "PM_STATE_WORK_TO_STANDBY", "PM_STATE_STANDBY_TO_SLEEP",
    "PM_STATE_STANDBY_TO_WORK", "PM_STATE_SLEEP_TO_WORK"
};
...
#if defined(CONFIG_SAMPLE_SUPPORT_MOUSE_BODY)
void uapi_pm_work_resume(void)
{
...
{
    if (g_pm_state_trans_func[PM_STATE_WORK_TO_STANDBY] == NULL || g_pm_cur_state == PM_STATE_STANDBY) {
        return;
    }
#if defined(CONFIG_SAMPLE_SUPPORT_MOUSE_BODY)
    extern bool mouse_body_pm_func(void);
    if(mouse_body_pm_func() == false)
    {
        return;
    }
#endif
...
{
    if (g_pm_state_trans_func[PM_STATE_STANDBY_TO_SLEEP] == NULL || g_pm_cur_state == PM_STATE_SLEEP) {
        return;
    }
#if defined(CONFIG_PM_SYS_SUPPORT_MSGQUEUE)
    pm_notify_mail_t mail = {0};
...
static void pm_state_sleep_to_work(void)
{
    ...
    g_pm_standby_base_tick_cnt = PM_CPU_CUR_TICK_CNT;
    g_pm_cur_state = PM_STATE_WORK;
#if defined (CONFIG_SAMPLE_SUPPORT_Tempt_TianSi)
    pm_state_work_to_standby();
#endif
}

#if defined (CONFIG_SAMPLE_SUPPORT_Tempt_TianSi)
void uapi_pm_state_standby_to_sleep(void)
{
...
static void pm_sys_task(void const *arg)
{
    ...
    while (1) {
        ...
        /* 二次判断，防止消息被打断，状态更新不能及时同步。 */
        uint32_t irq_status = osal_irq_lock();
        if ((mail.pm_state == PM_STATE_WORK_TO_STANDBY) &&
            ((PM_CPU_CUR_TICK_CNT - g_pm_work_base_tick_cnt) < g_pm_work_duration_ms)) {
            if (g_pm_cur_state == PM_STATE_STANDBY) {
                g_pm_cur_state = PM_STATE_WORK;
            }
            osal_irq_restore(irq_status);
            continue;
        }
        if ((mail.pm_state == PM_STATE_STANDBY_TO_SLEEP &&
            ((PM_CPU_CUR_TICK_CNT - g_pm_standby_base_tick_cnt) < g_pm_standby_duration_ms))) {
            if (g_pm_cur_state == PM_STATE_SLEEP) {
                g_pm_cur_state = PM_STATE_STANDBY;
            }
            osal_irq_restore(irq_status);
            continue;
        ...
        osal_printk("[pm_sys]mail.state switch: %s\n", pm_reverse_state[mail.pm_state]);
    }
}
#endif
...
#if defined(CONFIG_PM_SYS_SUPPORT) && (defined(CONFIG_SAMPLE_SUPPORT_MOUSE_BODY) || defined(CONFIG_SAMPLE_SUPPORT_Tempt_TianSi))
