...
{
    uint16_t wk_us = 4000;  // 4000：基准提前时间
#if defined(SLEM_CARKEY)
    wk_us = 4200;  // 4200：tr5310pa ec基准提前时间
#endif
#if defined(CONFIG_PM_POWER_GATING_ENABLE) && (CONFIG_PM_POWER_GATING_ENABLE == 1)
#if defined(CONFIG_SFC_SUPPORT_LPM)
    wk_us += 200;   // 200：SFC恢复
...
#endif
#if (defined(PM_SLEEP_DEBUG_ENABLE) && (PM_SLEEP_DEBUG_ENABLE == YES)) || defined (CONFIG_SAMPLE_SUPPORT_Tempt_TianSi)
