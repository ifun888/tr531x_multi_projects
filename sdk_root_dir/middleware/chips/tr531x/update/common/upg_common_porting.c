...
#include "non_os.h"
#include "cpu_utils.h"
#if (UPG_CFG_VERIFICATION_SUPPORT == YES) && (UPG_CFG_VERIFICATION_MODE_SW_HASH == NO)
#include "otp.h"
...
#define FLASH_SECTOR_ERASE_DELAY_MS    5
...
static upg_image_partition_ids_map_t g_img_partition_map[] = {
    ...
    { SLP_IMAGE_ID, PARTITION_SLP_IMAGE },
#endif
    { NV_BACKUP_IMAGE_ID, PARTITION_NV_DATA },
};
...
#define UPDATE_IMAGE_SET \
    { FLASHBOOT_IMAGE_ID, \
      ...
      NV_BACKUP_IMAGE_ID, \
      UPG_IMAGE_ID_NV }
#define UPDATE_IMAGE_SET_CNT 5 /* 镜像ID列表长度 */
#else
#define UPDATE_IMAGE_SET \
    { FLASHBOOT_IMAGE_ID, \
      APPLICATION_IMAGE_ID, \
...
static uint32_t g_tr5310p_support_upg_id[] = UPDATE_IMAGE_SET;
static upg_image_collections_t g_tr5310p_upg_collect = { g_tr5310p_support_upg_id, UPDATE_IMAGE_SET_CNT };
/* 获取当前程序支持的升级镜像 */
upg_image_collections_t *uapi_upg_get_image_id_collection(void)
{
    return &g_tr5310p_upg_collect;
}
...
static errcode_t flash_write(uint32_t addr, uint32_t size, uint8_t *data)
{
    ...
    if (addr >= EXTERN_FLASH_UPG_START_ADDRESS) {
        if (uapi_flash_write_data(0, (addr - EXTERN_FLASH_PARTITION_OFFSET), data, size) == size) {
            ret = ERRCODE_SUCC;
        }
    } else {
...
#else
    ret = uapi_sfc_reg_write(addr, data, size);
#endif
    return ret;
}
...
#if defined(SUPPORT_EXTERN_FLASH)
    if (addr >= EXTERN_FLASH_UPG_START_ADDRESS) {
        if (uapi_flash_read_data(0, (addr - EXTERN_FLASH_PARTITION_OFFSET), data, size) == size) {
            ret = ERRCODE_SUCC;
        }
    } else {
...
#else
    ret = uapi_sfc_reg_read(addr, data, size);
#endif
...
static errcode_t flash_erase(uint32_t addr, uint32_t size)
{
    ...
    if (addr >= EXTERN_FLASH_UPG_START_ADDRESS) {
        ...
        for (uint32_t i = addr; i < (addr + size); i += SIZE_4K) {
            ...
            osal_irq_restore(status);
            osal_msleep(FLASH_SECTOR_ERASE_DELAY_MS);
        }
#if defined(SUPPORT_EXTERN_FLASH)
    }
#endif
...
#if defined(SUPPORT_EXTERN_FLASH)
    if (addr >= EXTERN_FLASH_UPG_START_ADDRESS) {
        if (uapi_flash_block_erase(0, (addr - EXTERN_FLASH_PARTITION_OFFSET), size, true) == ERRCODE_SUCC) {
            ret = ERRCODE_SUCC;
        }
    } else {
#endif
