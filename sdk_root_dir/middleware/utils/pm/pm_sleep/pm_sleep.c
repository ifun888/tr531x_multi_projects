...
#if defined(CONFIG_PM_POWER_GATING_ENABLE) && defined(PM_SLEEP_DEBUG_ENABLE) && \
(CONFIG_PM_POWER_GATING_ENABLE == 1) && (PM_SLEEP_DEBUG_ENABLE == 1)
#define PM_PC_RECORD_NUM             20
static volatile uint32_t g_rstpc_debug[PM_PC_RECORD_NUM] = {0};
...
static pm_sleep_funcs_t g_pm_sleep_funcs;
...
{
#if defined(CONFIG_PM_SLEEP_RECORD_ENABLE) && (CONFIG_PM_SLEEP_RECORD_ENABLE == 1)
#if defined(CONFIG_PM_SLEEP_RECORD_USE_COUNT) && (CONFIG_PM_SLEEP_RECORD_USE_COUNT == 1)
    #define PM_GET_CURRENT_TIME PM_GET_CURRENT_COUNT
#else
    #define PM_GET_CURRENT_TIME PM_GET_CURRENT_MS
#endif
    if (info == SLP_INFO_TIME_BASE) {
        g_sleep_info.sleep_base_time = PM_GET_CURRENT_TIME;
        ...
        g_sleep_info.sleep_history[PM_SLEEP_LS].total_slp_time +=
            (PM_GET_CURRENT_TIME - g_sleep_info.sleep_base_time);
...
{
#if defined(CONFIG_PM_POWER_GATING_ENABLE) && (CONFIG_PM_POWER_GATING_ENABLE == 1)
#if defined(PM_SLEEP_DEBUG_ENABLE) && (PM_SLEEP_DEBUG_ENABLE == 1)
    g_pc_cnt = 0;
#endif
    uapi_pm_dev_suspend();
...
#if defined(PM_SLEEP_DEBUG_ENABLE) && (PM_SLEEP_DEBUG_ENABLE == 1)
    g_rstpc_debug[g_pc_cnt++] = PM_PC_COUNT_RECORD0;
#endif
    pm_cpu_suspend();
#if defined(PM_SLEEP_DEBUG_ENABLE) && (PM_SLEEP_DEBUG_ENABLE == 1)
    g_rstpc_debug[g_pc_cnt++] = PM_PC_COUNT_RECORD1;
#endif
    if (g_pm_stop_state == PM_SLEEP_STATE_GOTO_DS) {
#if defined(PM_SLEEP_DEBUG_ENABLE) && (PM_SLEEP_DEBUG_ENABLE == 1)
        g_rstpc_debug[g_pc_cnt++] = PM_PC_COUNT_RECORD2;
#endif
        g_pm_stop_state = PM_SLEEP_STATE_WKUP_PRE;
        pm_allow_deepsleep(true);
        pm_enter_wfi();
#if defined(PM_SLEEP_DEBUG_ENABLE) && (PM_SLEEP_DEBUG_ENABLE == 1)
        g_rstpc_debug[g_pc_cnt++] = PM_PC_COUNT_RECORD3;
#endif
...
#if defined(PM_SLEEP_DEBUG_ENABLE) && (PM_SLEEP_DEBUG_ENABLE == 1)
    g_rstpc_debug[g_pc_cnt++] = PM_PC_COUNT_RECORD4;
