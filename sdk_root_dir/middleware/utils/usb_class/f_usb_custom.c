...
#define USB_CUSTOM_IN_ENDPIONT_MAXNUM 1

#define STRING_PROPERTY_NAME_LEN 42
#define STRING_GUID_LEN 70
#define USB_DISCONNECT_DELAY 20
struct usb_os_msft_proerty_desc
{
    uint32_t dwSize;
    uint32_t dwPropertyDataType;
    uint32_t wPropertyNameLength;
...
struct test_dev_s
{
  struct usbdev_ep_s *in_ep[USB_CUSTOM_IN_ENDPIONT_MAXNUM];
  bool in_ep_enabled;
...
static const char g_ftest_str_manufacturer[] =
{
  26,
  USB_DESC_TYPE_STRING,
  'T', 0, 'r', 0, 'i', 0, 'd', 0, 'u', 0, 'c', 0, 't', 0, 'o', 0, 'r', 0, '-', 0, 'V', 0, '1', 0
};
...
static const char g_ftest_str_interface[TEST_STR_IDX_INTERFACE] =
{
  ...
  USB_DESC_TYPE_STRING,
  'C', 0, 'U', 0, 'S', 0, ' ', 0, 'I', 0, 'n', 0, 't', 0,
  'e', 0, 'r', 0, 'f', 0, 'a', 0, 'c', 0, 'e', 0
};

#define OS_STRING_SIGN_LEN 18
static const char g_custom_msft_string[OS_STRING_SIGN_LEN] = 
{
    OS_STRING_SIGN_LEN,
    USB_DESC_TYPE_STRING,
    'M', 0, 'S', 0, 'F', 0, 'T', 0,
...
#define DEF_MSFT_STRING {0xEE, g_custom_msft_string}
static struct usbd_string g_ftest_device_strings[8] =
{
  { 0, g_ftest_str_lang },
...
static struct usb_ifdesc_s g_ftest_intf_desc =
{
  ...
  .neps     = 1+USB_CUSTOM_IN_ENDPIONT_MAXNUM,    /* Number of endpoint */
  .classid  = 0xff,
  .subclass = 0xff,
  .protocol = 0xff,
...
static struct usb_epdesc_s g_ftest_in_interupt_ep_desc =
{
  .len      = sizeof(struct usb_epdesc_s),
  .type     = USB_DESC_TYPE_ENDPOINT,
  .addr     = USB_DIR_IN | 0x03,
  .attr     = 0x03,
  HSETW(.mxpacketsize, 0x040),
  .interval = 0
};

#define TEST_DESC_ARRAY_MAX_NUM 6
...
#if USB_CUSTOM_IN_ENDPIONT_MAXNUM > 1
  (const uint8_t *)&g_ftest_in_interupt_ep_desc,
#endif
  NULL,
};
...
ssize_t ftest_recv_data(char *buf, size_t buflen)
{
  ...
  if (test_data == NULL)
    {
      usb_warn("test_data is null!\r\n");
      return -1;
    }

 if (test_data->connected == 0)
    {
      usb_warn("test is disconnected!\r\n");
      return -1;
    }
  ...
  if (ret_event & USB_TEST_EXIT_EVENT)
    {
     usb_warn("ret_event: USB_TEST_EXIT_EVENT!!!\r\n");
      return -2;
    }
...
ssize_t ftest_send_data(const char *buf, size_t buflen, uint8_t in_endpoint_index)
{
  struct test_dev_s *test_data = g_test_dev;
  ...
  if (test_data->write_busy)
    ...
    if(in_endpoint_index > USB_CUSTOM_IN_ENDPIONT_MAXNUM - 1)
    {
     return -1;
    }
  ...
  req->len = buflen;
  if(test_data->in_ep[in_endpoint_index] == NULL)
  {
   return -1;
  }
  (void)EP_SUBMIT(test_data->in_ep[in_endpoint_index], req);
...
static int test_ep_data_init(struct test_dev_s *test_data, struct usbdev_s *dev, struct usbdev_devinfo_s *devinfo)
{
 ...
 /*in endpoint init*/
  for (uint8_t i = 0; i < TEST_DESC_ARRAY_MAX_NUM && g_ftest_desc_array[i] != NULL; ++i)
  {
   const uint8_t *des_src = g_ftest_desc_array[i];
    if(*(des_src + 1) != USB_DESC_TYPE_ENDPOINT)
    {
     continue;
    }
    struct usb_epdesc_s *endpoint_desc = (struct usb_epdesc_s *)g_ftest_desc_array[i];
    if((endpoint_desc->addr >> 7) == 0x00)
    {
     continue;
    }
    ep = DEV_ALLOCEP(dev, endpoint_desc->addr, (struct usb_epdesc_s *)endpoint_desc);
     if (ep == NULL)
       {
         return -1;
       }
...
static void test_ep_data_deinit(struct test_dev_s *test_data, struct usbdev_s *dev)
{
  ...
  if (test_data->out_ep != NULL)
    ...
    for(uint8_t i =0 ;i<USB_CUSTOM_IN_ENDPIONT_MAXNUM; i++)
    {
     if (test_data->in_ep[i] != NULL)
     {
       DEV_FREEEP(dev, test_data->in_ep[i]);
       test_data->in_ep[i] = NULL;
     }
    }
}
  ...
  if (test_data->in_ep_enabled == true)
    {
      for(uint8_t i =0; i< USB_CUSTOM_IN_ENDPIONT_MAXNUM; i++)
      {
       if(test_data->in_ep[i] != NULL)
       {
         (void)EP_DISABLE(test_data->in_ep[i]);
       }
      }
   ...
   for (uint8_t i = 0; i < TEST_DESC_ARRAY_MAX_NUM && g_ftest_desc_array[i] != NULL; ++i)
   {
     ...
     ret = EP_CONFIGURE(test_data->in_ep[index], (const struct usb_epdesc_s *)endpoint_desc, 0);
     index++;
     if (ret < 0)
       {
         usb_err("Config in_ep failed!\r\n");
         goto errout;
       }
   }  
...
#define STRING_PROPERTY_NAME_LEN 42
#define STRING_GUID_LEN 70
static const struct usb_os_msft_proerty_desc g_custom_msft_property = 
{
...
static int custom_get_msft_property_desc(uint8_t *buf)
{
    struct usb_msft_os_extprop_hdr_s *custom_hdr = (struct usb_msft_os_extprop_hdr_s *)buf;
    ...
    custom_hdr->len[0] = totallen;
    custom_hdr->version[1] = 1;
    custom_hdr->index[0] = MSFTOSDESC_INDEX_EXTPROP;
    custom_hdr->count[0] = 1;
    ret = memcpy_s(buf+sizeof(struct usb_msft_os_extprop_hdr_s), USB_COMP_EP0_BUFSIZ-sizeof(struct usb_msft_os_extprop_hdr_s),
                   &g_custom_msft_property, sizeof(struct usb_os_msft_proerty_desc));
    if(ret != EOK) {
                               ...
                               const struct usb_ctrlreq_s *ctrl, uint8_t *dataout, size_t outlen)
{
  ...
  uint16_t len = USB_GETW(ctrl->len);
  int ret = -1;
  ...
  if ((req_type & USB_REQ_TYPE_MASK) == USB_REQ_TYPE_STANDARD)
    ...
    ret = custom_get_msft_property_desc(req->buf);
  }
  if(ret>=0){
    req->len = MIN(ret, len);
...
static int16_t test_mkcfgdesc(uint8_t *buf, struct usbdev_devinfo_s *devinfo, uint8_t speed, uint8_t type)
{
  uint16_t total_len;
  uint8_t *des;
  errno_t ret;
...
void usbdev_test_initialize_sub(struct composite_devdesc_s *dev, int ifnobase, int minor)
{
