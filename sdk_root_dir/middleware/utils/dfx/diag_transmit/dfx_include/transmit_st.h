 ...
 */
#ifndef TRANSMIT_ST_H
#define TRANSMIT_ST_H
...
} transmit_save_info_t; /* transmit type is TRANSMIT_TYPE_FILE_DOWNSTREAM */

typedef struct {
    uint32_t start_addr;     /* 读取Flash数据的起始地址 */
} transmit_save_data_info_t; /* transmit type is TRANSMIT_TYPE_FLASH_UPSTREAM */
...
typedef struct {
    ...
    uint16_t pad        : 11;
    uint16_t transmit_type;         /* see transmit_type_t */
    uint32_t total_size;
    uint32_t info_size;
    uint8_t info[0];
