...
STATIC errcode_t transmit_dst_item_update_data(transmit_item_t *item, uint32_t offset, uint8_t *buf, uint32_t size)
{
    ...
    if (len <= 0) {
        ...
        item->file_fd = dfx_file_open_for_rewrite(item->local_file_name);
        if (item->file_fd < 0) {
            dfx_log_err("[ERR][transmit dst]file: %s reopen failed, fd = %d\r\n", item->local_file_name, item->file_fd);
    ...
    if (item->received_size == item->total_size) {
        ...
        transmit_record_progress(item->transmit_type, item->received_size);
#endif
...
errcode_t transmit_dst_item_process_data_reply_frame(transmit_data_reply_pkt_t *reply, diag_option_t *option)
{
    ...
    if (item == NULL) {
        ...
        return transmit_send_invalid_id(reply->transmit_id, option);
    }
...
STATIC bool transmit_item_check_free_space(transmit_item_t *item)
{
    ...
    for (int i = 0; i < DFX_FILE_LEN; i++) {
        file_path[i] = (uint8_t)(item->local_file_name[i]);
...
errcode_t transmit_dst_item_process_start_frame(transmit_start_pkt_t *start_pkt, diag_option_t *option)
{
    ...
    if (start_pkt->info_size != 0) {
        ...
        transmit_item_init_local_file_name(item, save_file_info->file_name, save_file_info->name_size);
    }
    item->last_rcv_pkt_time = cur_time;
...
/* 作为上位机目的端（接收并保存数据的一端）处理NEGOTIATE ACK帧 */
errcode_t transmit_dst_host_process_negotiate_ack_frame(transmit_negotiate_ack_pkt_t *negotiate_ack_pkt)
{
    dfx_assert(negotiate_ack_pkt);
    uint32_t cur_time = dfx_get_cur_second();
    transmit_item_t *item = transmit_item_match_id(negotiate_ack_pkt->transmit_id);
    if (item == NULL) {
        dfx_log_err("[ERR][transmit host dst] found item failed\r\n");
        return ERRCODE_FAIL;
    }

    transmit_item_init_data_block_size(item, negotiate_ack_pkt->data_block_size);
    transmit_item_init_data_block_number(item, negotiate_ack_pkt->data_block_number);
    item->last_rcv_pkt_time = cur_time;
    return transmit_dst_item_start_transmit_request(item);
}
#endif
