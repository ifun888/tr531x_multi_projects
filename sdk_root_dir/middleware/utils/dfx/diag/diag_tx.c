...
#if defined(CONFIG_DFX_SUPPORT_OFFLINE_LOG_FILE) && (CONFIG_DFX_SUPPORT_OFFLINE_LOG_FILE == DFX_YES)
...
STATIC errcode_t diag_pkt_router_tx(const diag_channel_item_t *chan, diag_pkt_handle_t *pkt)
{
...
errcode_t diag_pkt_router_output(diag_pkt_handle_t *pkt, const diag_pkt_process_param_t *process_param)
{
    ...
    diag_channel_item_t *chan = diag_dst_2_chan(dst);
    return diag_pkt_router_tx(chan, pkt);
}
