...
#define DIAG_FILTER_GROUP_NUM 8
#define DIAG_STATE_CONNECT_NOTIFY_CNT 3
...
typedef struct {
    ...
    uint16_t enable_id[CONFIG_DIAG_FILTER_GROUP_NUM];
} zdiag_filter_ctrl_t;
...
STATIC zdiag_filter_notify_hook g_zdiag_notify_process[DIAG_STATE_CONNECT_NOTIFY_CNT];
...
void zdiag_set_enable(bool enable, diag_addr addr)
{
    ...
    for (i = 0; i < DIAG_STATE_CONNECT_NOTIFY_CNT; i++) {
...
void zdiag_set_level_enable(uint8_t level, bool enable)
{
    ...
    if (level >= DIAG_FILTER_GROUP_NUM) {
        return;
    }
...
void zdiag_set_id_enable(uint32_t id, bool enable)
{
    ...
    zdiag_filter_ctrl_t *ctrl = zdiag_get_filter_ctrl();
    for (i = 0; i < CONFIG_DIAG_FILTER_GROUP_NUM; i++) {
