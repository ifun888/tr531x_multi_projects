...
int zdiag_mocked_shell_uart_puts(uint8_t *data, uint32_t data_len)
{
    ...
    uint8_t *print_buf = (uint8_t *)dfx_zalloc(0, header_len + data_len);
    if (print_buf == NULL) {
        return 0;
    }
    ...
    diag_channel_item_t *chan = diag_dst_2_chan(header->dst);
...
{
    uint8_t *out_buf = (uint8_t *)dfx_zalloc(0, SIZEBUF);
    if (out_buf == NULL) {
        return -1;
    }
