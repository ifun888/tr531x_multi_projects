...
{
    errcode_t err = uapi_nv_write_with_attr(key, kvalue, kvalue_length, NULL, NULL);
#ifdef CONFIG_ERRCODE_SUPPORT_REPORT
    uapi_errcode_invoke_report((uint8_t)ERRCODE_MODULE_NV, err);
#endif
    return err;
}
...
{
    nv_key_attr_t attr;
    errcode_t err = uapi_nv_read_with_attr(key, kvalue_max_length, kvalue_length, kvalue, &attr);
#ifdef CONFIG_ERRCODE_SUPPORT_REPORT
    uapi_errcode_invoke_report((uint8_t)ERRCODE_MODULE_NV, err);
#endif
    return err;
