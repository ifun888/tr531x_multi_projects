...
static void set_sbc_enc_definition_param(acodec_get_private_param *arg, aenc_attr *attr,
    sbc_enc_config *sbc_enc_param)
{
    sbc_enc_config *enc_config = NULL;
    acodec_get_private_param *param = NULL;
    sbc_enc_config *definition_param = sbc_enc_param;
    ...
    param = (acodec_get_private_param *)arg;

    enc_config = (sbc_enc_config *)param->private_attr;
    enc_config->num_of_subbands = definition_param->num_of_subbands;
    enc_config->msbc = definition_param->msbc;
    enc_config->channel_mode = definition_param->channel_mode;
    enc_config->sample_rate = definition_param->sample_rate;
    enc_config->num_of_blocks = definition_param->num_of_blocks;
    enc_config->allocation_method = definition_param->allocation_method;
    enc_config->bit_pool = definition_param->bit_pool;
    param->private_attr_size = (uint32_t)sizeof(sbc_enc_config);

    attr->open_param.private_data = arg;
    attr->open_param.private_data_size = sizeof(*arg);
}

static void aenc_set_sbc_param(aenc_param_cfg *sbc_enc_param_ctrl, int32_t codecid)
{
    sbc_enc_param_ctrl->param_flag = SBC_PARAM_FLAG;
    sbc_enc_param_ctrl->sbc_enc_param.num_of_subbands = SUBBANDS_8;
    if (codecid == HA_CODEC_ID_MSBC) {
        sbc_enc_param_ctrl->sbc_enc_param.msbc = TRUE;
        sbc_enc_param_ctrl->sbc_enc_param.channel_mode = SBC_ENC_MONO;
        sbc_enc_param_ctrl->sbc_enc_param.sample_rate = SBC_SF16000;
        sbc_enc_param_ctrl->sbc_enc_param.num_of_blocks = MSBC_CAPS_DEFAULT_NUM_OF_BLOCKS;
        sbc_enc_param_ctrl->sbc_enc_param.allocation_method = SBC_ALLOC_LOUDNESS;
        sbc_enc_param_ctrl->sbc_enc_param.bit_pool = 26; /* 26 bit pool num */
    } else {
        sbc_enc_param_ctrl->sbc_enc_param.msbc = FALSE;
        sbc_enc_param_ctrl->sbc_enc_param.channel_mode = SBC_ENC_STEREO;
        sbc_enc_param_ctrl->sbc_enc_param.sample_rate = SBC_SF48000;
        sbc_enc_param_ctrl->sbc_enc_param.num_of_blocks = SBC_BLOCK_16;
        sbc_enc_param_ctrl->sbc_enc_param.allocation_method = SBC_ALLOC_LOUDNESS;
        sbc_enc_param_ctrl->sbc_enc_param.bit_pool = 30; /* 30 bit pool num */
    }
    return;
}
...
int32_t aenc_open_codec(int32_t codecid, ha_codec_enc_param *open_param)
{
    ...
    if (codecid == HA_CODEC_ID_SBC || codecid == HA_CODEC_ID_MSBC) {
        aenc_set_sbc_param(&aenc_inst->sbc_enc_param_ctrl, codecid);
    }

    if (aenc_inst->sbc_enc_param_ctrl.param_flag == USE_DEFINITION) {
        set_sbc_enc_definition_param(&arg, &attr, &aenc_inst->sbc_enc_param_ctrl.sbc_enc_param);
    } else {
