...
errcode_t uapi_upg_full_image_update(const upg_image_header_t *image)
{
    ...
    if (ret_val != ERRCODE_SUCC) { return ret_val; }

    if (app_size < image->image_len) { return ERRCODE_FAIL; }

    uint32_t read_len = 0;
    uint32_t tmp_len = 0;

    uint8_t *buffer = upg_malloc(UPG_FLASH_PAGE_SIZE);
    if (buffer == NULL) { return ERRCODE_MALLOC; }
    ...
    upg_free(buffer);
    buffer = NULL;
#if (UPG_CFG_SUPPORT_BOOT_IMAGE_PROTECTION == YES)
    uint32_t flag = UPG_BOOT_JUMP_FLAG;
    uint32_t flag_len = sizeof(flag);
    if (image->image_id == UPG_BOOT_IMAGE_ID) {
        ret = upg_write_new_image_data(UPG_BOOT_JUMP_FLAG_PAGE_ADDR + UPG_BOOT_JUMP_FLAG_OFFSET, (uint8_t *)&flag,
                                       &flag_len, image->image_id, false);
    }
#endif
    return ret;
}
