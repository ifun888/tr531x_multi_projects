                                          ...
                                          upg_image_status_t status, fota_upgrade_flag_area_t *upg_flag)
{
    ...
    ret = upg_perform_image_backup(img_header->image_id);
    if (ret != ERRCODE_SUCC) {
        return ret;
    }
#endif
...
#if (UPG_CFG_SUPPORT_BOOT_IMAGE_PROTECTION == YES)
    if (image_id == UPG_BOOT_IMAGE_ID && write_offset == UPG_BOOT_JUMP_FLAG_PAGE_ADDR) {
        uint32_t *val = (uint32_t *)(buffer + UPG_BOOT_JUMP_FLAG_OFFSET);
        *val = UPG_BOOT_NO_JUMP_FLAG;
    }
#endif
