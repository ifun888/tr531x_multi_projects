...
typedef struct {
    td_float x;
    td_float y;
} ext_vau_affine_translate_attr;

typedef struct {
    td_float x_multiplier;
    td_float y_multiplier;
} ext_vau_affine_zoom_attr;

typedef struct {
    td_s32 degree;
} ext_vau_affine_rotate_attr;

typedef struct {
    ext_vau_reflect_mode mode;
} ext_vau_affine_reflect_attr;

typedef struct {
    td_float x;
    td_float y;
} ext_vau_affine_shearing_attr;
...
typedef enum {
    EXT_MAPPING_OPT_MATRIX = 0x0,    /*!< @if Eng matrix trans.
                                          @else   矩阵变换 @endif */
    EXT_MAPPING_OPT_MESH,            /*!< @if Eng mesh mode.
                                          @else   mesh模式 @endif */
    EXT_MAPPING_OPT_SPHERE,          /*!< @if Eng sphere mode.
                                          @else   球面模式 @endif */
    EXT_MAPPING_OPT_PLANE,           /*!< @if Eng pattern mode.
                                          @else   平面模式 @endif */
    EXT_MAPPING_OPT_MAX              /*!< @if Eng Invalid value.
                                          @else   无效值CNend @endif */
} ext_vau_mapping_mode;
...
/**
 * @if Eng
 * @brief  Gaussian quality parameter.
 * @else
 * @brief  高斯质量参数, 质量越好，性能越差
 * @endif
 */
typedef enum {
    EXT_VAU_BLUR_QUALITY_L = 0,
    EXT_VAU_BLUR_QUALITY_M,
    EXT_VAU_BLUR_QUALITY_H,
    EXT_VAU_BLUR_QUALITY_MAX
} ext_vau_blur_quality;

/**
 * @if Eng
 * @brief  blur area.
 * @else
 * @brief  模糊区域: 矩形区域&绘制蒙版&蒙版素材
 * @endif
 */
typedef enum {
    EXT_VAU_BLUR_AREA_RECT = 0,
    EXT_VAU_BLUR_AREA_MASK_DRAW,
    EXT_VAU_BLUR_AREA_MASK_IMG,
    EXT_VAU_BLUR_AREA_MAX
} ext_vau_blur_area;

/**
 * @if Eng
 * @brief  Gaussian Attributes.
 * @else
 * @brief  高斯模糊属性
 * @endif
 */
...
typedef struct {
    ...
    td_float sigma_x;
    td_float sigma_y;
    ext_vau_blur_area area;
    ext_vau_blur_mode blur_mode;
    ext_vau_blur_quality blur_quality;
#if TARGET_CHIP_BRANDY
	ext_vau_gaussian_para gaussian_attr;
#endif
} ext_vau_blur_opt;
 ...
 */
typedef struct {
    td_bool mapping_en;                  /*!< @if Eng enable.
                                              @else   映射使能 @endif */
    ext_vau_mapping_mode mapping_mode;   /*!< @if Eng mapping mode.
                                              @else   映射模式 @endif */
} ext_vau_mapping_attr;
...
/**
 * @if Eng
 * @brief  Lighting Mode Attributes.
 * @else
 * @brief  光照模式属性
 * @endif
 */
...
typedef enum {
    EXT_VAU_CIRCLE_RADIAL    = 0x0,
    EXT_VAU_CIRCLE_CONST     = 0x1,
    EXT_VAU_FAN_RADIAL       = 0x2,
    EXT_VAU_FAN_CONST        = 0x3,
    EXT_VAU_FAN_SWEEP_RADIAL = 0x4,
    EXT_VAU_FAN_SWEEP_CONST  = 0x5,
} ext_vau_lighting_mode;

typedef struct {
    ext_vau_image_mode image_mode;
    td_float cx;
    td_float cy;
    td_float radius;
    td_float spe_amp; // 镜面反射强度
    td_float amb_amp; // 环境光强度
    td_float start_angle_rad;
    td_float start_end_angle_rad;
    td_float end_start_angle_rad;
    td_float end_angle_rad;
    td_bool is_bi_dir;
} ext_vau_lighting_sw;

typedef struct {
    td_bool enable;                             /*!< @if Eng enable.
                                                     @else   使能 @endif */
    ext_vau_lighting_mode mode;                 /*!< @if Eng Lighting model.
                                                     @else   光照模式 @endif */
    ext_vau_lighting_sw attr;                   /*!< @if Eng Lighting parameter.
                                                     @else   光照参数 @endif */
} ext_vau_lighting_opt;
 ...
 */
typedef struct {
    td_bool colorize_en;               /*!< @if Eng color rize mode attribute.
                                            @else   color rize 模式属性 @endif */
    ...
    ext_vau_lighting_opt lighting_opt; /*!< @if Eng Lighting attribute.
                                            @else   lighting 模式属性 @endif */
} ext_vau_blit_opt;
...
typedef enum {
    ...
    EXT_VAU_PATTERN_PAD     = 0x2,    /*!< @if Eng pattern pad.
                                           @else   pad填充 @endif */
    EXT_VAU_PATTERN_REPEAT  = 0x3,    /*!< @if Eng pattern repeat.
                                           @else   repeat填充 @endif */
    EXT_VAU_PATTERN_REFLECT = 0x4,    /*!< @if Eng pattern reflect.
                                           @else   reflect填充 @endif */
...
typedef struct {
    td_bool hvs_en;
    td_bool mask_en;
    td_u8 mask_threshold; // [0, 255]: 8-bit
    td_s16 h_adj;         // [-359, 359]: 10-bit
    td_s8 s_adj;          // [-100, 100]: 8-bit
    td_s8 v_adj;          // [-100, 100]: 8-bit
} ext_vau_hsv_info;
 ...
 */
typedef enum {
    EXT_SAMPLING_DIRECT = 0x0,    /*!< @if Eng Direct.
                                       @else   点映射 @endif */
    EXT_SAMPLING_HLINEAR,         /*!< @if Eng HLinear sampling.
                                       @else   水平线性采样 @endif */
    EXT_SAMPLING_VLINEAR,         /*!< @if Eng VLinear sampling.
                                       @else   垂直线性采样 @endif */
    EXT_SAMPLING_BILINEAR,        /*!< @if Eng Bilinear sampling.
                                       @else   双线性 @endif */
    EXT_SAMPLING_MAX
} ext_vau_sampling_flags;
...
typedef struct {
    ...
    td_u32 phy_addr;
    td_uchar *vir_addr;
} ext_vau_model_surface;

typedef struct {
    td_float angle;
    td_float v_fov;
    td_float aspect_ratio;
    td_float z_near;
    td_float z_far;
    ext_vau_vec3 axis;
    ext_vau_vec3 scale;
    ext_vau_vec3 translation;
    ext_vau_vec3 look_from;
} ext_vau_camera_attr;

typedef struct {
    td_bool enable;
    ext_vau_color bg_color;
    ext_vau_color shadow_color;
    ext_vau_sampling_flags mapping_color;
    ext_vau_sampling_flags mapping_alpha;
    ext_vau_sampling_flags model_pos;
    ext_vau_sampling_flags model_flag;
    ext_vau_camera_attr camera_attr;
    ext_vau_model_surface model_sur;
} ext_vau_3d_attr;
...
typedef struct {
    ...
    ext_vau_draw_attr *draw;             /*!< @if Eng The ES is not used currently.
                                              @else   ES暂未使用 @endif */
    ext_vau_subnode subnode;
    ext_vau_hsv_info hsv_info;
    ext_vau_mapping_attr mapping_attr;
    ext_vau_3d_attr *threed_attr;
} ext_vau_surface;
