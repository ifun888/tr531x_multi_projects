...
static uint8_t g_sever_addr_rpt[SLE_SERVER_ADDR_RPT_LEN] = { 0x1F, 0x0, 0x0, 0x0, 0x0, 0x01, 0x0C, 0x09, 0x00,
                                                             0x03, 0x06, 0x00 };
static uint8_t g_sever_data_rpt[SLE_SERVER_DATA_RPT_LEN] = { 0x1F, 0x0, 0x0, 0x0, 0x0, 0x01, 0x0C, 0x09, 0x00,
                                                             0x09, 0x06, 0x00 };
...
static void sle_ota_client_sample_seek_result_data(sle_seek_result_info_t *seek_result_data)
{
    if (seek_result_data->data_length <= 0) {
        return;
    }
    if (seek_result_data->data_length > (SLE_SERVER_DATA_RPT_LEN - SLE_SERVER_DATA_INDEX)) {
        osal_printk("seek result data length is too long!\r\n");
        return;
    }
    for (uint8_t i = 0; i < seek_result_data->data_length; i++) {
        g_sever_data_rpt[SLE_SERVER_DATA_INDEX + i] = (uint8_t)seek_result_data->data[i];
    }
    /* TLV body len: type 1byte  + length 2bytes + data length */
    uint16_t body_len = sizeof(uint8_t) + sizeof(uint16_t) + seek_result_data->data_length;
    g_sever_data_rpt[SLE_SERVER_RPT_LEN_INDEX] = body_len;
    g_sever_data_rpt[SLE_SERVER_RPT_LEN_INDEX + 1] = body_len >> SLE_SERVER_8_BIT_SHIFT;
    g_sever_data_rpt[SLE_SERVER_DATA_LEN_INDEX] = seek_result_data->data_length;
    g_sever_data_rpt[SLE_SERVER_DATA_LEN_INDEX + 1] = seek_result_data->data_length >> SLE_SERVER_8_BIT_SHIFT;
    fhid_send_data(sle_ota_get_hid_index(), (char *)g_sever_data_rpt, SLE_SERVER_DATA_RPT_LEN);
}
...
static void sle_ota_client_sample_seek_result_info_cbk(sle_seek_result_info_t *seek_result_data)
{
    ...
    if ((find_connect_server = sle_ota_find_unconnect_server_by_addr(seek_result_data->addr.addr))
        ...
        if (get_sle_ota_scan_state() == true) {
            ...
            sle_ota_client_sample_seek_result_data(seek_result_data);
        }
    }
}
