...
static uint8_t g_local_name[ NAME_MAX_LENGTH] = { '5', '3', '1', 'X', '-', 'p', 'm', '-', 'd', 'e', 'm', 'o','\0'};
...
uint8_t pm_ble_start_adv(void)
{
    ...
    uint8_t adv_addr[BD_ADDR_LEN] = {0x08,0x01,0x02,0x03,0x04,0x10};
    bd_addr_t device_addr;
    device_addr.type = BT_ADDRESS_TYPE_PUBLIC_DEVICE_ADDRESS;
    memcpy_s(&device_addr.addr, BD_ADDR_LEN, adv_addr, BD_ADDR_LEN);
    ...
    memcpy_s(&adv_para.own_addr, sizeof(bd_addr_t), &device_addr, sizeof(bd_addr_t));
    ...
    // gap_ble_get_local_addr(&device_addr);
    osal_printk("device_addr:");
    for (uint8_t i = 0; i < BD_ADDR_LEN; i++)
    {
        osal_printk("0x%02x", device_addr.addr[i]);
    }
    osal_printk("\r\n");
    osal_printk("device_own_addr:");
    for (uint8_t i = 0; i < BD_ADDR_LEN; i++)
    {
        osal_printk("0x%02x", adv_para.own_addr.addr[i]);
    }
    osal_printk("\r\n");
    return 0;
}
