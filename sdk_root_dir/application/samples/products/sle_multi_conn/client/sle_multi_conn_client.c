...
static void sle_multi_conn_client_read_rssi_cbk(uint16_t conn_id, int8_t rssi, errcode_t status)
{
    osal_printk("%s conn_id:%x, rssi=%d, status=%d", SLE_MULTI_CONN_CLIENT_LOG, conn_id, rssi, status);
}
...
static void sle_multi_conn_client_sample_connect_cbk_register(void)
{
    ...
    g_sle_multi_conn_connect_cbk.read_rssi_cb = sle_multi_conn_client_read_rssi_cbk;
