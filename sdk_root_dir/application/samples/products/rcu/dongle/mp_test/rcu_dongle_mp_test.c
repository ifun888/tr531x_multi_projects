...
void rcu_dongle_mp_test_set_test_station(uint8_t *tlv)
{
    ...
    if (sle_set_nv_smp_keys(&g_key[g_current_test_station - 1], &g_own_addr[g_current_test_station - 1],
        &peer_addr, 0)!= ERRCODE_SLE_SUCCESS) {
        osal_printk("Set test station fail.\r\n");
    }
#endif
...
{
    unused(arg);
    osal_printk("rcu dongle rcu_dongle_mp_test_task\r\n");
