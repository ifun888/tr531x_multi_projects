...
static errcode_t sle_bas_property_and_descriptor_add(void)
{
    ...
    ret = sle_add_property(properties, g_sle_bat_uuid[SLE_BAT_INDEX1], sizeof(g_battert_value),
...
errcode_t sle_rcu_bas_service_add(void)
{
    ...
    if (sle_bas_property_and_descriptor_add() != ERRCODE_SLE_SUCCESS) {
        ssaps_unregister_server(sle_get_server_id());
        return ERRCODE_SLE_FAIL;
    }

    ret = ssaps_start_service(sle_get_server_id(), g_bas_service_hdl[SLE_BAT_INDEX0].handle_out);
    if (ret != ERRCODE_SLE_SUCCESS) {
        return ERRCODE_SLE_FAIL;
    }
