...
static void sle_enable_cbk(uint8_t status)
{
    ...
    if (sle_rcu_easy_connect_server_add() != ERRCODE_SLE_SUCCESS) {
        osal_printk("%s sle_rcu_easy_conn_server_init, sle_rcu_server_add fail\r\n", SLE_RCU_SERVER_LOG);
        return;
    }
...
errcode_t sle_rcu_announce_register_cbks(void)
{
    ...
    seek_cbks.seek_enable_cb    = sle_sample_seek_enable_cbk;
    seek_cbks.seek_disable_cb   = sle_sample_seek_disable_cbk;
    seek_cbks.seek_result_cb    = sle_sample_seek_result_info_cbk;
