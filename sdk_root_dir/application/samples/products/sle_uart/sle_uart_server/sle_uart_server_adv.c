...
/* 连接调度间隔4ms，单位125us */
#define SLE_CONN_INTV_MIN_DEFAULT                 0x20
/* 连接调度间隔4ms，单位125us */
#define SLE_CONN_INTV_MAX_DEFAULT                 0x20
/* 连接调度间隔25ms，单位125us */
...
/* 低时延模式下不允许休眠连接间隔 */
#define SLE_CONN_MAX_LATENCY                      0x0
/* 广播发送功率 */
...
/* 广播名称 */
static uint8_t sle_local_name[NAME_MAX_LENGTH] = "sle_uart_s_test";
...
static int sle_set_default_announce_param(void)
{
    ...
    uint8_t index;
    unsigned char local_addr[SLE_ADDR_LEN] = { 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0x01 };
...
errcode_t sle_device_register_cbks(void)
{
    errcode_t ret = 0;
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle_device_register_cbks,register_callbacks fail :%x\r\n",
