...
#ifdef CONFIG_SAMPLE_SUPPORT_LOW_LATENCY_TYPE
    g_trigger = 1;
    sle_low_latency_set(conn_id, true, SLE_LOW_LATENCY_2K);
...
static void sle_server_set_phy_cbk(uint16_t conn_id, errcode_t status, const sle_set_phy_t *param)
{
    osal_printk("set phy callback exec success. conn id:%d status:%d\r\n",conn_id, status);
    osal_printk("tx format:%d rx format:%d tx phy:%d\r\n", param->tx_format, param->rx_format, param->tx_phy);
    osal_printk("rx phy:%d tx pilot:%d rx pilot:%d\r\n", param->rx_phy, param->tx_pilot_density, param->rx_pilot_density);
}
...
static errcode_t sle_conn_register_cbks(void)
{
    ...
    conn_cbks.set_phy_cb = sle_server_set_phy_cbk;
    ...
    ssaps_write_callback)
{
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle_uart_server_init,sle_low_latency_rx_enable fail :%x\r\n", SLE_UART_SERVER_LOG, ret);
        return ret;
    }
#endif
