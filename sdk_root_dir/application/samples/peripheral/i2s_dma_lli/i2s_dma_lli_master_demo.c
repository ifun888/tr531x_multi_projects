...
#else
#define I2S_TX_INT_THRESHOLD        0xF
#endif
...
static void *i2s_dma_master_task(const char *arg)
{
    unused(arg);
    uint32_t val = 0;
    uint32_t ret = CONFIG_I2S_TRANSFER_LEN_OF_DMA_LLI;
    ...
    while (1) {
        uapi_watchdog_kick();
        val = uapi_i2s_merge_write_by_dma(SIO_BUS_0, &g_i2s_send_dma_data, CONFIG_I2S_TRANSFER_LEN_OF_DMA_LLI, &dma_cfg,
            (uintptr_t)NULL, true);
        if (val != ret) {
            osal_printk("uapi_i2s_merge_write_by_dma error, val: 0x%x.\r\n", val);
        }
    }
    return NULL;
}
