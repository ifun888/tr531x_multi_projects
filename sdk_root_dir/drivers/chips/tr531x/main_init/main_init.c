...
#endif
#include "soc_osal.h"
#if defined(SUPPORT_CHIP_N1200) && defined(SUPPORT_REMOTE_LIB)
...
static void app_upg_init(void)
{
    ...
    upg_func.malloc = irmalloc;
    upg_func.free = irfree;
...
{
    uint32_t soft_rst_2 = readl(0x570000B8);
    if ((soft_rst_2 & 0x2) != 3) {   // 0x2 means lowest 2 bit reset setting, 3 means 2 bits are set
        /* below deassert BT subsys */
...
static void chip_hw_init(void)
{
    ...
    osal_printk("Debug uart init succ! Flash Size:0x%x\r\n", FLASH_LEN);
 ...
 *  app_os_init():  线程初始化，详细定义请参考 drivers/chips/tr531x/main_init/app_os_init.c 文件。
 */
