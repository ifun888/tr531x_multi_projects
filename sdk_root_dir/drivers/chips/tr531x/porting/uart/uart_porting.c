...
#if (defined (CONFIG_DRIVERS_USB_SERIAL_GADGET)) && (defined (SW_UART_DEBUG))
...
{
    unused(bus);
#if defined (BUILD_LOADBOOT) || defined(BUILD_ROMBOOT) || defined(BUILD_FLASHBOOT) || defined(SUPPORT_CHIP_N1200)
    return CLK_32M;
#else
    if (bus == UART_BUS_1) {
...
void sw_debug_uart_init(uint32_t baud_rate)
{
    ...
    (void)uapi_uart_init(g_sw_debug_uart, NULL, &uart_line_config, NULL, &uart_buffer_config);
