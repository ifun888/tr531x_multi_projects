...
errcode_t uapi_clock_control(clock_control_type_t type, uint8_t param)
{
    ...
    switch (type) {
        ...
        case CLOCK_CONTROL_SLP_FREQ_CONFIG:
#endif
...
#if defined(CONFIG_PM_CLOCK_SUPPORT_SFC_HIGH_FREQ)
__attribute__((section(".PMRAMCODE"))) void clock_sfc_enable_high_freq(void)
{
    reg32_setbit(0x57008410, 0x3); // XO2USB enable
    writel(0x57004780, 0x2); // USB phy vdd使能
    writel(0x57004788, 0x1); // USB ISO使能
    writel(0x57004780, 0x3); // USB上电
    writel(0x57004788, 0x0); // USB ISO关闭
    writel(0x5700478C, 0x4); // USB复位
    writel(0x5700478C, 0x5); // USB复位
    writel(0x5700478C, 0x7); // USB复位
    writel(0x57000828, 0x2); // USB总线复位
    writel(0x57000828, 0x3); // USB总线复位
    writel(0x57000900, 0x2); // POR复位
    writel(0x57000904, 0x0); // UTMI复位
    writel(0x57000824, 0x1); // USB phy test时钟使能
    writel(0x57000824, 0x3); // USB phy test复位
    writel(0x57018004, 0x85000000); // usb_phy内部寄存器
    writel(0x57018100, 0x402); // ref_clk选择

    reg32_clrbit(0x52000548, 0x7);
    writel(0x5200060C, 0x25);
    writel(0x5200060C, 0x65);
    writel(0x5200060C, 0xE5);
    uint32_t clk_cg = readl(0x52000548);
    writel(0x52000548, (clk_cg | 0x380)); // 开启时钟CG
}
#endif