...
errcode_t uapi_i2s_init(sio_bus_t bus, i2s_callback_t callback)
{
    ...
    hal_sio_init(bus);
    hal_sio_register_callback(bus, (i2s_callback_t)callback);
...
errcode_t uapi_i2s_deinit(sio_bus_t bus)
{
    ...
    hal_sio_unregister_callback(bus);
    hal_sio_deinit(bus);

    sio_porting_clock_enable(false);
    g_i2s_inited = false;
#if defined(CONFIG_I2S_SUPPORT_DMA)
    osal_sem_destroy(&(g_tx_dma_trans[bus].dma_sem));
    osal_sem_destroy(&(g_rx_dma_trans[bus].dma_sem));
#endif
...
errcode_t uapi_i2s_set_config(sio_bus_t bus, const i2s_config_t *config)
{
    ...
    hal_sio_set_config(bus, &hal_config);
    return ERRCODE_SUCC;
}
...
errcode_t uapi_i2s_get_config(sio_bus_t bus, i2s_config_t *config)
{
    ...
    errcode_t ret = hal_sio_get_config(bus, &g_hal_config);
...
errcode_t uapi_i2s_get_data(sio_bus_t bus, i2s_rx_data_t *data)
{
    ...
    if (unlikely((bus >= CONFIG_I2S_BUS_MAX_NUM) || (data == NULL))) {
        return ERRCODE_INVALID_PARAM;
    }

    i2s_rx_data_t *temp_data = data;

    hal_sio_get_data(bus, (hal_sio_rx_data_t *)(uintptr_t)temp_data);
...
errcode_t uapi_i2s_write_data(sio_bus_t bus, i2s_tx_data_t *data)
{
    ...
    if (unlikely((bus >= CONFIG_I2S_BUS_MAX_NUM) || (data == NULL) || (data->left_buff == NULL) ||
        (data->right_buff == NULL))) {
        return ERRCODE_INVALID_PARAM;
    }

    hal_sio_write(bus, (hal_sio_tx_data_t *)(uintptr_t)data, I2S_MODE);
...
errcode_t uapi_i2s_loop_trans(sio_bus_t bus, i2s_tx_data_t *data)
{
    ...
    hal_sio_start_loop_trans(bus, (hal_sio_tx_data_t *)(uintptr_t)data, I2S_MODE);
...
int32_t uapi_i2s_dma_config(sio_bus_t bus, i2s_dma_attr_t *i2s_dma_cfg)
{
    ...
    if (unlikely(bus >= CONFIG_I2S_BUS_MAX_NUM || i2s_dma_cfg == NULL)) {
        return ERRCODE_INVALID_PARAM;
    }

    hal_sio_set_dma_config(bus, (hal_i2s_dma_attr_t *)i2s_dma_cfg);
    ...
    i2s_dma_config_t *dma_cfg, uintptr_t arg, bool block)
{
    ...
    if ((bus >= I2S_MAX_NUMBER) || (buffer == NULL) || (length == 0) || (dma_cfg == NULL)) {
        return ERRCODE_INVALID_PARAM;
    }
