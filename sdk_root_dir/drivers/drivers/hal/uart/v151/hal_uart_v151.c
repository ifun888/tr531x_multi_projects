...
#define IBRD_EIGHT_NEED_BAUD_OFFSET_NUM          3
#define REMAINDER_EIGHT_NEED_BAUD_OFFSET_NUM     3
...
STATIC const hal_uart_ctrl_t g_hal_uart_ctrl_func_array[UART_CTRL_MAX] = {
    ...
    hal_uart_en_para_err_int,              // UART_CTRL_EN_OVERRUN_ERR_INT
...
{
    fifo_ctl_t data = (fifo_ctl_t)g_hal_uart_reg[uart]->fifo_ctl;
#if defined(CONFIG_UART_IP_VERSION_V151_PRO) && defined(CONFIG_UART_DOUBLE_WATERLINE)
    waterline_ctl_t water = (waterline_ctl_t)g_hal_uart_reg[uart]->waterline_ctl;
    water.waterline_ctl.double_wl_enable = 1;  // enable double water level
    g_hal_uart_reg[uart]->waterline_ctl = water.d32;
    uart_double_water_t *double_water = uart_port_get_double_water_level_value(uart);
    if (double_water != NULL) {
        data.fifo_ctl.rx_empty_trig_h = double_water->rx_empty_trig_h;
        data.fifo_ctl.rx_empty_trig_l = double_water->rx_empty_trig_l;
    }
#endif
...
void hal_uart_set_baud_rate(uart_bus_t uart, uint32_t baud, uint32_t uart_clk)
{
    ...
    div_h_t div_h;
    div_l_t div_l;
    uint16_t ibrd_need_baud_offset;
    uint32_t remainder_need_baud_offset;
    if (baud == 0) {
        return;
    }

    if ((baud << IBRD_NEED_BAUD_OFFSET_NUM) <= uart_clk) {
#if defined(CONFIG_UART_IP_VERSION_V151_PRO)
        hal_uart_set_lp_req_en(uart, true);
        hal_uart_set_uniform_div_en(uart, true);
#endif
        hal_uart_set_baud_div(uart, HAL_UART_SAMPLE_BAUD_RATE_SIXTEEN, HAL_UART_RX_SAMPLE_PHASE_SIXTEEN);
        ibrd_need_baud_offset = IBRD_NEED_BAUD_OFFSET_NUM;
        remainder_need_baud_offset = REMAINDER_NEED_BAUD_OFFSET_NUM;
    } else {
#if defined(CONFIG_UART_IP_VERSION_V151_PRO)
        hal_uart_set_lp_req_en(uart, false);
        hal_uart_set_uniform_div_en(uart, true);
#endif
        hal_uart_set_baud_div(uart, HAL_UART_SAMPLE_BAUD_RATE_EIGHT, HAL_UART_RX_SAMPLE_PHASE_EIGHT);
        ibrd_need_baud_offset = IBRD_EIGHT_NEED_BAUD_OFFSET_NUM;
        remainder_need_baud_offset = REMAINDER_EIGHT_NEED_BAUD_OFFSET_NUM;
    }

    uint16_t brd_i = (uint16_t)(uart_clk / (baud << ibrd_need_baud_offset));
    uint32_t remainder = uart_clk % (baud << remainder_need_baud_offset);
    uint16_t brd_f = (uint16_t)round_off(((uint64_t)remainder << UART_DLF_SIZE) / (baud << ibrd_need_baud_offset));
...
/**
 * uart baud sample switch need config both rx sample pahse and Sample baud rate.
 */
void hal_uart_set_baud_div(uart_bus_t uart, uint8_t sample_baud_rate, uint8_t rx_sample_phase)
{
    ...
    data.baud_ctl.baud_div = sample_baud_rate;
    data.baud_ctl.sample_phase = rx_sample_phase;
...
/**
 * uart v151 pro uniform division is configured when the UART uses fractional part of divisor.
 */
void hal_uart_set_uniform_div_en(uart_bus_t uart, bool val)
{
    uniform_div_ctl_t data = (uniform_div_ctl_t)g_hal_uart_reg[uart]->uniform_div_ctl;
    data.uniform_div_ctl.uniform_div_enable = val;
    g_hal_uart_reg[uart]->uniform_div_ctl = data.d32;
}
#endif

STATIC void hal_uart_line_status_error_intr_handler(uart_bus_t uart, uint32_t line_status)
{
    ...
    g_hal_uart_callback[uart](uart, UART_EVT_ID_INVALID, 0);
}
...
void hal_uart_irq_handler(uart_bus_t uart)
{
...
#endif
#if defined(CONFIG_UART_IP_VERSION_V151_PRO)
        // read fifo Status register to clear this interrupt
        hal_uart_get_fifo_status(uart);
#endif
        g_hal_uart_callback[uart](uart, UART_EVT_FRAME_ERR_ISR, 0);
    }
    ...
    if ((uart_int_reg.intr_id.intr_id == HAL_UART_INTERRUPT_CHAR_TIMEOUT) ||
    ...
    if (uart_int_reg.intr_id.intr_id == HAL_UART_INTERRUPT_RECEIVER_LINE_STATUS) {
        ...
        uint32_t uart_line_status = hal_uart_get_error_status(uart);
        hal_uart_line_status_error_intr_handler(uart, uart_line_status);
    }
}
