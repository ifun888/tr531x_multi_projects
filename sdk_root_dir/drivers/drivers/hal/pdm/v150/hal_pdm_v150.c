...
static errcode_t hal_pdm_v150_set_attr(uint8_t mic, hal_pdm_config_t *attr)
{
    ...
    if (mic_tmp == HAL_PDM_AMIC || mic_tmp == HAL_PDM_DMIC_0) {
        if (mic_tmp == HAL_PDM_AMIC) {
            hal_pdm_v150_ch_ctrl_set_mic_sel(1);
            hal_pdm_v150_hpf_ctrl_set_hpf_fs_sel(mic_tmp, 1);
        } else {
    ...
    hal_pdm_v150_up_fifo_st_ctrl_set_up_fifo_aempty_int_en(1);
    hal_pdm_v150_up_fifo_st_ctrl_set_up_fifo_empty_int_en(1);
...
{
    return hal_pdm_v150_fifo_get_addr();
}

static uint32_t hal_pdm_v150_get_fifo_deepth(void)
{
    return hal_pdm_v150_fifo_get_deepth();
}
