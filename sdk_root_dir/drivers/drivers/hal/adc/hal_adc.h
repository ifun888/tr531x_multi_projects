...
errcode_t hal_adc_init(void);
errcode_t hal_adc_deinit(void);
errcode_t hal_adc_ch_set(adc_channel_t channel, bool on);
#if defined(CONFIG_ADC_SUPPORT_DIFFERENTIAL)
errcode_t hal_adc_diff_ch_set(adc_channel_t postive_ch, adc_channel_t negative_ch, bool on);
#endif
void hal_adc_power_en(afe_scan_mode_t afe_scan_mode, bool on);
int32_t hal_adc_manual(adc_channel_t channel);
int32_t hal_adc_auto_sample(adc_channel_t channel);
