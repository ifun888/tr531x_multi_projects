...
def get_group_owner(data_dict, save_name, target_name):
    ...
    with open(OUTPUT_DIR / f"{save_name}_library_summary.csv", 'w', encoding='utf_8_sig') as f:
       ...
       # 各组规格限制 PLAT WIFI BTC BTH UNKNOWN  STACK PKTRAM
        lim_all = {'tr5336-liteos-app.map_ram': {'PLAT' : 35.50, 'WIFI':46.6, 'RADAR':49, 'BTC': 20,'BTC_CHBA': 1, 'BTH': 10, 'UNKNOWN': 0.5, 'STACK':7,'PKTRAM':50},
            'control_trswgxx.map_ram':{'PLAT' : 17.1, 'WIFI':21.5, 'BTC': 32, 'BTH': 10, 'UNKNOWN': 0.1, 'STACK':6,'HEAP':27},
            'control_trswgxx.map_flash':{'PLAT' : 11, 'WIFI':56, 'BTC': 108, 'BTH': 6, 'UNKNOWN': 0.1},
            'application.map_flash':{'PLAT' : 217.099, 'WIFI':670, 'BTC': 1, 'BTC_CHBA': 16, 'BTH': 217, 'UNKNOWN': 5.65},
            'application.map_ram':{'PLAT' : 44.5, 'WIFI':56, 'BTC': 1, 'BTC_CHBA': 1, 'BTH': 5, 'STACK':13, 'UNKNOWN': 3.1},
            'tr5336-liteos-app.map_flash':{'PLAT' : 202, 'WIFI':645, 'RADAR':52, 'BTC': 163.5, 'BTC_CHBA': 16, 'BTH': 246, 'UNKNOWN': 0.5}}
        target_list = {'trswgxx_control_app', 'trswgxx_liteos_app', 'tr5336_liteos_app'}
        lim_i = 0
    ...
    with open(OUTPUT_DIR / f"{save_name}_file_summary.csv", 'w', encoding='utf_8_sig') as f:
        header = ['Group_Owner', 'Size', 'Limit']
        f.write(f"{','.join(header)}\n")
        lim_all = {'control_trswgxx.map_flash':{'系统组' : 7.8, '协议组':17.1, '应用组': 2.0, '前端组': 27.8, '算法组': 1.4,'WIFI_UNKNOWN': 0.1},
            'application.map_flash':{'系统组' : 35.5, '协议组':217.001, '应用组': 319, '前端组': 16, '算法组': 39,'WIFI_UNKNOWN': 0.1}}
        target_list = {'trswgxx_control_app', 'trswgxx_liteos_app'}
        lim_i = 0
