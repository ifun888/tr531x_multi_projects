    ...
    if (NOT ${SEC_BOOT} STREQUAL "")
        if (${SEC_BOOT_VERIFY})
            if(EXISTS ${ROOT_DIR}/interim_binary/${CHIP}/bin/boot_bin/${SEC_BOOT}/sec_boot_sign.bin)
                # 存在secboot sign则不签名secboot，只签名flashboot
                ...
                # 不存在secboot sign则签名secboot和flashboot
...
if (${CHIP} STREQUAL "tr5336")
add_custom_target(TR5336_GENERAT_SIGNBIN ALL
    COMMAND ${Python3_EXECUTABLE} ${ROOT_DIR}/build/config/target_config/${CHIP}/sign_config/params_and_bin_sign.py ${TARGET_NAME}
    COMMENT "tr5336 image sign"
    WORKING_DIRECTORY ${PROJECT_BINARY_DIR}
    DEPENDS GENERAT_BIN
)

if(TARGET GENERAT_ROM_PATCH)
    add_dependencies(TR5336_GENERAT_SIGNBIN GENERAT_ROM_PATCH)
endif()
endif()

if (${CHIP} STREQUAL "trswgxx")
add_custom_target(TRSWGXX_GENERAT_SIGNBIN ALL
    COMMAND sh ${ROOT_DIR}/build/config/target_config/${CHIP}/sign_config/params_and_bin_sign.sh ${BUILD_TARGET_NAME} ${BIN_NAME}
    COMMENT "trswgxx image sign"
    WORKING_DIRECTORY ${PROJECT_BINARY_DIR}
    DEPENDS GENERAT_BIN
)

if(TARGET GENERAT_ROM_PATCH)
    add_dependencies(TRSWGXX_GENERAT_SIGNBIN GENERAT_ROM_PATCH)
endif()
endif()
