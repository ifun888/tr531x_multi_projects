...
g_carkey_role_list = ["standard-tr5310pa-slekey-can-master-n1200", "standard-tr5310pa-slekey-can-slave-n1200",
                      "standard-tr5310pa-slekey-can-hub-n1200", "standard-tr5310pa-slekey-key-n1200"]
...
def make_all_in_one_packet(pkg_soc, pack_style_str, sector_cfg):
    ...
    if sector_cfg:
        ...
        if pack_style_str.startswith("pack"):
            ...
            glp_dir =   os.path.join(interim_binary_dir, "slp", pack_style_str, "file_array_ota")
            output_dir = os.path.join(SDK_DIR, "output", pkg_soc, "fwpkg", pack_style_str)

        # fwpkg output dir
        if pack_style_str in g_carkey_role_list:
            tr531x_fwpkg = os.path.join(output_dir, "%s_all_in_one_%s.fwpkg" %(pkg_soc, pack_style_str.split("-")[-2]))
            tr531x_flashboot_only_fwpkg = os.path.join(output_dir, "%s_flashboot_only_%s.fwpkg" %(pkg_soc, pack_style_str.split("-")[-2]))
            tr531x_loadapp_only_fwpkg = os.path.join(output_dir, "%s_loadapp_only_%s.fwpkg" %(pkg_soc, pack_style_str.split("-")[-2]))
            tr531x_nv_fwpkg = os.path.join(output_dir, "%s_nv_only_%s.fwpkg" %(pkg_soc, pack_style_str.split("-")[-2]))
        else:
            tr531x_fwpkg = os.path.join(output_dir, "%s_all_in_one.fwpkg" %pkg_soc)
            tr531x_flashboot_only_fwpkg = os.path.join(output_dir, "%s_flashboot_only.fwpkg" %pkg_soc)
            tr531x_loadapp_only_fwpkg = os.path.join(output_dir, "%s_loadapp_only.fwpkg" %pkg_soc)
            tr531x_nv_fwpkg = os.path.join(output_dir, "%s_nv_only.fwpkg" %pkg_soc)
        tr531x_ide_fwpkg = os.path.join(SDK_DIR, "tools", "pkg", "fwpkg", pkg_soc, "%s_all.fwpkg" %pkg_soc)
        tr531x_efuse_fwpkg = os.path.join(output_dir, "%s_efuse.fwpkg" %pkg_soc)
        ...
        if os.path.isfile(slp):
            if info["slp"][1] == 0:
                print("no need to pack slp.")
            elif os.path.getsize(slp) < info["slp"][1]:
    ...
    try:
        ...
        if os.path.isfile(slp) and info["slp"][1] > 0:
            packet_post_agvs.append(slp_bx)
        packet_bin(tr531x_fwpkg, packet_post_agvs)
        packet_bin(tr531x_ide_fwpkg, packet_post_agvs)
        ...
        if flashboot_bak_bx is not None:
            packet_post_agvs.append(flashboot_bak_bx)
        packet_bin(tr531x_flashboot_only_fwpkg, packet_post_agvs)

        # pack application only fwpkg.
        packet_post_agvs = list()
        packet_post_agvs.append(loadboot_bx)
        packet_post_agvs.append(app_bx)
        packet_bin(tr531x_loadapp_only_fwpkg, packet_post_agvs)

        # pack nv only fwpkg.
        packet_post_agvs = list()
        packet_post_agvs.append(loadboot_bx)
        packet_post_agvs.append(nv_bx)
        packet_bin(tr531x_nv_fwpkg, packet_post_agvs)
        ...
        if os.path.isfile(efuse):
            ...
            packet_bin(tr531x_efuse_fwpkg, packet_post_agvs)
