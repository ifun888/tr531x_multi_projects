 ...
 */
typedef enum {
    GAP_BLE_PRIVACY_DISABLE,     /*!< @if Eng Disable privacy, address do not use RPA
                                 @else   关闭隐私功能，地址不使用RPA地址 @endif */
    GAP_BLE_PRIVACY_ENABLE,      /*!< @if Eng Enable privacy, local address use RPA
 ...
 */
typedef enum {
    BLE_DISCONNECT_UNKNOWN                 = 0x00,    /*!< @if Eng disconnect by local
                                                         @else   未知原因断链 @endif */
    BLE_DISCONNECT_BY_PIN_OR_KEY_MISSING   = 0x06,    /*!< @if Eng disconnect by local
                                                         @else   pin或key丢失 @endif */
    BLE_DISCONNECT_BY_CONNECT_TIMEOUT      = 0x08,     /*!< @if Eng disconnect by local
                                                         @else   连接超时断链 @endif */
    BLE_DISCONNECT_BY_REMOTE_USER          = 0x13,    /*!< @if Eng disconnect by remote
                                                         @else   远端用户断链 @endif */
    BLE_DISCONNECT_BY_LOCAL_HOST           = 0x16,    /*!< @if Eng disconnect by remote
                                                         @else   本端HOST断链 @endif */
    BLE_DISCONNECT_BY_MIC_ERROR            = 0x3D,    /*!< @if Eng disconnect by remote
                                                         @else   mic errr断链 @endif */
    BLE_ESTABLISH_CONNECT_FAIL             = 0x3E,    /*!< @if Eng disconnect by remote
                                                         @else   建连异常 @endif */
...
/**
 * @if Eng
 * @brief Struct of BLE scan result data.
 * @else
 * @brief 扫描结果数据。
 * @endif
 */
...
/**
 * @if Eng
 * @brief Struct of BLE link update event parameters.
 * @else
 * @brief ACL链路更新事件参数
 * @endif
 */
...
typedef struct {
    ...
    uint8_t privacy;           /*!< @if Eng use le privacy { @ref gap_ble_privacy_t }
                                    @else   使用隐私功能 { @ref gap_ble_privacy_t } @endif */

} gap_ble_sec_params_t;

/**
 * @if Eng
 * @brief Struct of BLE authentication result.
 * @else
 * @brief BLE认证结果。
 * @endif
 */
