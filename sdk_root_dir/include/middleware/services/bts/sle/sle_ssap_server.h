...
typedef struct {
    uint16_t request_id;  /*!< @if Eng Request id.
                               @else   请求id。 @endif */
    uint16_t begin_handle;      /*!< @if Eng Properity handle of the read request.
                               @else   请求读的属性句柄。 @endif */
    uint16_t end_handle;      /*!< @if Eng Properity handle of the read request.
                                  @else   请求读的属性句柄。 @endif */
    uint8_t type;         /*!< @if Eng property type { @ref ssap_property_type_t }.
                               @else   属性类型。 @endif  { @ref ssap_property_type_t } */
    sle_uuid_t uuid;             /*!< @if Eng UUID of SSAP descriptor.
...
typedef struct {
    ...
    ssaps_read_by_uuid_request_callback read_by_uuid_request_cb; /*!< @if Eng Read request received callback.
                                                                  @else   收到远端读请求回调函数。 @endif */
