 ...
 */
typedef struct {
    uint8_t  *n;            /*!< @if Eng RSA public modulur.
                                 @else   RSA 公共模数。 @endif */
    uint8_t  *e;            /*!< @if Eng RSA public exponent.
                                 @else   RSA 公共指数。 @endif */
    uint16_t len;           /*!< @if Eng RSA public key length. The max length is 512 bytes.
                                 @else   RSA公钥长度， 最大长度为512字节。 @endif */
