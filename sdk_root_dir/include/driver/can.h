...
/**
 * @if Eng
 * @brief  Query the RX pin level of the CAN controller.
 * @param  [in]  bus CAN bus line select.
 * @retval 0x0 Low level.
 * @retval 0x1 High level.
 * @retval Other CAN bus is not yet initialized.
 * @else
 * @brief  查询CAN控制器RX脚电平。
 * @param  [in]  bus CAN总线选择。
 * @retval 0x0 低电平。
 * @retval 0x1 高电平。
 * @retval Other CAN通路尚未初始化。
 * @endif
 */
uint8_t uapi_can_get_rx_sts(uint8_t bus);
